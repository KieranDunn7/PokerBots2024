        ...
        if street == 0:
            ...
            if continue_cost == BIG_BLIND - SMALL_BLIND:
                ...
                if self.total_percentage < 0.45:
                    return FoldAction()
                if self.total_percentage > 0.6 or self.total_percentage < 0.55:
                    return CallAction()
                return RaiseAction(random.choice([4, 5, 6]))
        ...
        if self.street_num < street: # new card dealt
            ...
            if board_flush:
                for suit_index, cards_in in board_flush.items():
                    ...
                    elif len(cards_in) == 3 and not self.board_flush_need_1 and not self.board_flush: # three in a suit on board
            ...
            board_straight = check_for_straight(board_cards, 3)
            if board_straight:
                for high_card, (cards_in, cards_out) in board_straight.items():
                    if len(cards_in) == 5: # full straight on board
                        if self.board_flush:
                            ...
                            self.board_straight = False
                            self.board_straight_need_1 = False
                            self.board_straight_need_2 = False
                            self.board_flush = False
                    ...
                    elif len(cards_in) == 4 and not self.board_straight: # four in straight on board
                    ...
                    elif len(cards_in) == 3 and not self.board_straight_need_1 and not self.board_straight: # three in a straight on board
...
################# FLOP #########################
        ...
        if street == 3: # flop post-auction
            ...
            if self.straight:
                ...
                elif self.board_trips: # need auction
                    return medium_raise
                
                elif self.board_two_pair: # need auction
                    return medium_raise
            ...
            if self.two_pair:
                if self.board_two_pair:
                    ...
                    if self.two_pair_ranks[0] > self.board_pair_rank and medium_flop_bet:
                        return CallAction()
                elif self.board_flush_need_2 or self.board_flush_need_2 and medium_flop_bet:
                    return CallAction()
                elif self.two_pair_ranks[0] >= self.sorted_board_ranks[0] or self.two_pair_ranks[0] >= self.sorted_board_ranks[2] and medium_flop_bet:
                    return CallAction()
                elif small_flop_bet:
                    return CallAction()
        ...
        if street == 4: # turn
            ...
            if continue_cost == 0 and big_blind:
                ...
                if self.straight:
                    ...
                    elif self.board_straight_need_1:
                ...
                if self.two_pair:
                    if self.board_two_pair:
                        ...
                        if self.two_pair_ranks[0] > self.board_pair_rank:
                            return small_raise
                    elif self.board_flush_need_2 or self.board_flush_need_2:
                        return small_raise
                    return medium_raise
            ...
            if my_pip == 0 and continue_cost <= max(5, pot_size/25):
                ...
                if self.two_pair:
                    if self.board_two_pair:
                        if self.my_high_card >= 10 or self.my_high_card >= 7 and not self.high_cards_or_pair_likely:
                            return medium_raise
                    elif self.board_flush_need_1 or self.board_straight_need_1 or self.board_flush_need_2 and opp_auction:
                        pass
                    elif self.board_pair:
                        if self.two_pair_ranks[0] > self.board_pair_rank:
                            return small_raise
                ...
                if self.flush_draw and self.straight_draw:
                    if self.board_flush_need_1 or self.board_straight_need_1 or self.board_flush_need_2 and opp_auction:
                        return action
                    elif self.my_flush_high >= 10 or len(self.draw_needed) >= 2 and self.my_straight_high > self.board_straight_high:
                        return small_raise
                
                if self.flush_draw:
                    if self.board_flush_need_1 or self.board_flush_need_2 and opp_hand_size == 3:
                        return CheckAction()
                    elif self.my_flush_high >= 10:
                        return small_raise
                    
                if self.straight_draw:
                    if len(self.draw_needed) == 2 and self.my_straight_high > self.board_straight_high and not (self.board_straight_need_1 or self.board_straight_need_2 and opp_auction):
                        return small_raise
            ...
            if big_blind and my_pip == 0:
                ...
                if self.flush_draw and self.straight_draw:
                    if self.board_flush_need_1 or self.board_straight_need_1 or self.board_flush_need_2 and opp_auction:
                        if small_turn_bet:
                            return CallAction()
                    elif medium_turn_bet:
                        return CallAction()
                    
                if self.flush_draw:
                    if self.board_flush_need_1 or self.board_two_pair:
                        pass
                    elif self.board_straight_need_1 or self.board_flush_need_2 and opp_auction:
                        if small_turn_bet:
                            return CallAction()
                    elif medium_turn_bet:
                        return CallAction()
                ...
                if self.straight_draw:
                    if self.board_straight_need_1 or self.board_two_pair or self.board_flush_need_2 and opp_auction:
                        pass
                    elif len(self.draw_needed) == 2 and medium_turn_bet:
                        return CallAction()
            ...
            if my_pip != 0:
                ...
                if self.flush:
                    ...
                    elif self.board_flush_need_2 and (self.my_flush_high >= 7 or self.my_flush_high >= 5 and not self.high_cards_or_pair_likely):
                        return medium_raise
                ...
                if self.straight:
                    ...
                    if self.board_flush_need_1:
                        ...
                        if opp_auction and small_turn_bet:
                            return CallAction()
                        elif medium_turn_bet:
                            return CallAction()
                ...
                if self.flush_draw:
                    if self.board_flush_need_1 or self.board_two_pair or self.board_flush_need_2 and opp_auction:
                        pass
                    elif self.board_straight_need_1:
                        if small_turn_bet:
                            return CallAction()
                ...
                if self.straight_draw:
                    if self.board_straight_need_1 or self.board_two_pair or self.board_flush_need_1 or self.board_trips or self.board_flush_need_2 and opp_auction:
                        pass
                    elif len(self.draw_needed) == 2 and small_turn_bet:
                        return CallAction()
            ...
            if self.two_pair:
                if self.board_two_pair:
                    ...
                    return CallAction()
                elif self.two_pair_ranks[0] >= self.sorted_board_ranks[0] or self.two_pair_ranks[2] >= self.sorted_board_ranks[0] and medium_turn_bet:
                    return CallAction()
                elif small_turn_bet:
                    return CallAction()
            ...
            if self.flush_draw:
                if self.board_flush_need_1:
                    pass
                elif self.board_straight_need_1 and small_turn_bet:
                    return CallAction()
                elif medium_turn_bet and not self.board_two_pair and not self.board_trips and not (self.board_flush_need_2 and opp_auction):
                    return CallAction()
...
###################### RIVER ############################
        ...
        if street == 5: # river
            ...
            if continue_cost == 0 and big_blind:
                ...
                if self.high_hand == 8:
                ...
                elif self.high_hand == 7:
                    if self.board_quads:
                        if self.my_high_card >= 10 or self.my_high_card >= 7 and not self.high_cards_or_pair_likely:
                            return high_raise
                        return small_raise
                    elif self.board_full_house or self.board_trips:
                        return high_raise
                    else:
                        return medium_raise
                
                elif self.high_hand == 6:
                ...
                elif self.high_hand == 5:
                    if self.board_flush:
                        if self.my_flush_high:
                            if self.my_flush_high > self.board_flush_min and self.my_flush_high > 4:
                                return medium_raise
                        return high_raise
                    
                    elif self.board_flush_need_1:
                ...
                elif self.high_hand == 4:
                    
                    if self.board_flush_need_1 and opp_auction:
                        pass
                    
                    elif self.board_straight:
                        if self.my_straight_high > self.board_straight_high:
                            return high_raise
                        return CheckAction()
                    elif self.board_flush_need_1 or self.board_flush_need_2 and opp_auction:
                        return small_raise
                    ...
                    elif self.board_straight_need_2 and self.board_straight_high:
                        return high_raise
            ...
            if my_pip == 0 and continue_cost <= max(5, pot_size/25):
                ...
                if self.high_hand == 8:
                    ...
                    if self.board_straight_need_2 and self.board_straight_high:
                        return high_raise
            ...
            if my_pip != 0:
                ...
                if self.high_hand == 8:
                    ...
                    elif self.board_flush_need_2 and self.my_flush_high > 5:
                        return high_raise
                    ...
                    if self.board_straight:
                        ...
                        if medium_river_bet:
                            return CallAction()
                        return action
                    elif self.board_flush_need_1 or self.board_trips or self.board_two_pair:
                        pass
                    elif self.board_pair:
                        return CallAction()
                    ...
                    elif self.board_straight_need_2 and self.my_straight_high > self.board_straight_high:
                        return high_raise
                    ...
                    if self.board_trips:
                        ...
                        pass
                    elif self.board_pair and medium_river_bet:
                        return CallAction()
                    else:
                        return CallAction()
                    ...
                    if self.board_pair or self.board_flush_need_1 or self.board_straight_need_1:
                        ...
                        if medium_river_bet:
                            return CallAction()
                    elif self.board_pair == self.sorted_board_ranks[1] and small_river_bet:
                        return CallAction()
